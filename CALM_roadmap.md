...
### 🐛 Исправленные баги (24 июля 2026)

### 7. Vulkan Q8_0 — субнормальные FP16 шкалы в шейдере

**Симптом:** `--backend vulkan` генерирует мусор ("12398547689720234563") вместо связного текста. CPU работает корректно.

**Причина:** В GLSL шейдере `shaders/q8_0_matmul.comp` функция `fp16_to_fp32_u()` имела неверную формулу для субнормальных FP16 значений (exp=0):
```glsl
// BUG
uint exp32 = uint(127 - 14 - s);  
// FIX  
uint exp32 = uint(127 - 24 + s);
```
Ошибка давала factor 4×–1024× для всех 991 субнормальных кодировок. В модели Q8_0 0.15-0.35% блоков имеют субнормальные шкалы — через 24 слоя ошибка накапливалась до 25% RMS diff, сдвигая top-1 токен.

**Фикс:** `shaders/q8_0_matmul.comp` строка 56, перекомпилирован SPIR-V и заголовок.

**Результат:** CPU vs VK max_diff упал с 0.418 → 0.000017. Vulkan теперь выдаёт связный английский текст. Оставшиеся различия — нормальные IEEE 754 между NEON FMA и GPU mul+add.

**Затронутые файлы:**
- `shaders/q8_0_matmul.comp`
- `shaders/q8_0_matmul.spv`
- `shaders/q8_0_matmul_spv.h`

**Документация:** `DOCS/vulkan_debug.md`
